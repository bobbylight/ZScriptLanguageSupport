...
public class Scanner {
	...
	public Scanner(Reader r) {
		s = r!=null ? new ZScriptScanner(r) : null;
		stack = new Stack<>();
	}
...
private void pushOntoStack(Token t) {
	 ...
	 */
	public Position createOffset(final int offs) {
		if (doc!=null) {
			try {
	...
	public void eatThroughNext(int tokenType) throws IOException {
		eatThroughNext(tokenType, true);
	}


	/**
	 * Eats all tokens up to the next token of the specified type.  This is
	...
	public void eatThroughNextSkippingBlocks(int tokenType) throws IOException {
		...
		while ((t=yylex())!=null) {
			...
			// Check this outside of switch so they can eat until end '}', for
			// example
			if (type==tokenType) {
				if (blockDepth<=0) {
					return;
	 ...
	 * @see #eatThroughNextSkippingBlocks(int, int)
	 */
	public Token eatThroughClosingParen() throws IOException {
		return eatThroughNextSkippingBlocksAndStuffInParensImpl(TokenTypes.SEPARATOR_RPAREN, -1, 1);
	}
	...
	public Token eatThroughNextSkippingBlocksAndStuffInParens(int tokenType1,
									int tokenType2) throws IOException {
		return eatThroughNextSkippingBlocksAndStuffInParensImpl(tokenType1, tokenType2, 0);
	}
...
private int currentResetStartOffset;
	...
	public Token yylexNonNull(int type1, int type2, int type3, String error)
		...
		if (t.getType()!=type1 && (type2==-1 || t.getType()!=type2) &&
			...
			throw new ParseException(t, error + ", found '" + t.getLexeme() + "'");
		}
		return t;
	}
	...
	public Token yyPeek(int depth) throws IOException {
		if (depth<1) {
			throw new /*IllegalArgumentException*/
					IOException("Internal error!  Stack depth must be >= 1");
		}
