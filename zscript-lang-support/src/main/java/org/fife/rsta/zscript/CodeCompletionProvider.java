	...
	private void getCompletionsArrowed(JTextComponent comp, String text,
        ...
        for (String global : globals) {
            if (global.equals(first)) {
                ...
                addGlobalMemberCompletions(global, text, retVal);
                return;
            }
        }
		...
		if (endsWithArrow) {
			...
			SortedSet<Completion> varCompletions = new TreeSet<>(completionComparator);
...
return null;
		...
		List<Completion> completionsAt = new ArrayList<>();
		...
		if (t!=null) {
			switch (t.getType()) {
				...
				case Token.VARIABLE:
					...
					List<? extends Completion> sourceList;
					if (source==null) {
	...
	private Completion getMatchingCompletion(List<? extends Completion> sourceList, String name) {
		CompletionComparator comparator = new CompletionComparator(c -> {
		...
		return index>=0 ? sourceList.get(index) : null;
	}
	 ...
	 */
	private static boolean isSequencePointChar(char ch) {
